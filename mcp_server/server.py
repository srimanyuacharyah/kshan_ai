...
async def get_world_resource(scenario_id: str, auth_token: str = "") -> str:
    """Read-only resource for world chronicle and realm topology."""
    async for db in get_mcp_db():
        return await resource_read_world(auth_token=auth_token, scenario_id=scenario_id, db=db)

@mcp_server.resource("kshan://timeline/{branch_id}")
async def get_timeline_resource(branch_id: str, auth_token: str = "") -> str:
    """Read-only resource for full timeline chronicle of a branch."""
    async for db in get_mcp_db():
        return await resource_read_timeline(auth_token=auth_token, branch_id=branch_id, db=db)

@mcp_server.resource("kshan://memories/{branch_id}")
async def get_memories_resource(branch_id: str, auth_token: str = "") -> str:
    """Read-only resource for all memory shards unlocked in a branch."""
    async for db in get_mcp_db():
        return await resource_read_memories(auth_token=auth_token, branch_id=branch_id, db=db)
